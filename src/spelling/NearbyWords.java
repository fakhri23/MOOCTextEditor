...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly) {
		...
		for (int index = 0; index <= s.length(); index++) {
			for (int charCode = (int) 'a'; charCode <= (int) 'z'; charCode++) {
				StringBuffer mutation = new StringBuffer(s);
				mutation.insert(index, (char) charCode);

				if (!currentList.contains(mutation.toString()) && (!wordsOnly || dict.isWord(mutation.toString()))
						&& !s.equals(mutation.toString())) {
					currentList.add(mutation.toString());
				}
			}
		}
	}
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while ( !queue.isEmpty()) {
			...
			this.substitution(currentWord, neighbors, true);
			this.insertions(currentWord, neighbors, true);
			this.deletions(currentWord, neighbors, true);
	...
	public static void main(String[] args) {
		...
		NearbyWords words = new NearbyWords(dict);
		List<String> OneMutation = words.distanceOne(word, true);
		System.out.println("One away word Strings for \"" + word + "\" are:");
		System.out.println(OneMutation + "\n");
		word = "kangaro";
		List<String> suggest = words.suggestions(word, 10);
		System.out.println("Spelling Suggestions for \"" + word + "\" are:");
		System.out.println(suggest);
