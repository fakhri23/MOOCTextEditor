...
public abstract class Document {
	 ...
	 *         pattern
	 */
	protected List<String> getTokens(String pattern) { //O(n) n = text.length() 
	...
	protected int countSyllables(String word) {
		...
		List<Character> voyels = new ArrayList<Character>();
		voyels.add('e');
		voyels.add('i');
		voyels.add('u');
		voyels.add('y');
		voyels.add('a');
		voyels.add('o');
		// System.out.println(voyels.contains('e'));
		int count = 0;
		for (int i = 0; i < word.length(); i++) { // O(n) 
			char letter = word.charAt(i);
			if ((i > 0 && i < word.length() - 1) || ((i == word.length() - 1 && letter != 'e') && word.length() > 1)) {
				if (voyels.contains(letter)) {
					char letterBefore = word.charAt(i - 1);
					if (!voyels.contains(letterBefore)) {
						count++;
					}
				}
			} else if (i == 0) {
