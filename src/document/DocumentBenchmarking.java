	...
	public static void main(String[] args) {
		...
		for (int numToCheck = start; numToCheck < numSteps * increment + start; numToCheck += increment) {
			...
			for (int i=0; i<trials; i++ ){
				...
				double fscoreb = bd.getFleschScore(); 
				double basicall = (System.nanoTime()-basicstart)/1000000000.0; 
				finb+=basicall; 
			}
				
			System.out.print(finb+"\t");
			
			double finef = 0.0; 
			for (int i=0; i<trials; i++){
				long effstart = System.nanoTime(); 
				Document ef = new EfficientDocument(text); 
				double fscoreef= ef.getFleschScore(); 
				double effcall = (System.nanoTime()-effstart)/1000000000.0; 
				finef +=effcall; 
			}
			System.out.println(finef); 
